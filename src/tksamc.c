        ...
        if (a!=AUX_MON)
          {
            ...
            Eold = Eold + Eij[AUX_MON][a]*cargaMon[AUX_MON]*cargaMon[a];
          }
       }
    ...
    for (a=1;a<=n;a++)
      {    
        if (a!=AUX_MON)
        {
          //Enew = Enew + 0.0005*Eij[AUX_MON][a]*cargaN[AUX_MON]*cargaN[a];
            Enew = Enew + Eij[AUX_MON][a]*cargaN[AUX_MON]*cargaN[a];
        }
      }
  }
     ...
     for (a=1;a<=n;a++)
     {
      ...
      for (k=1;k<=n;k++)
      {
       ...
       ET1 = ET1 + Eij[a][k]*cargaMon[a]*cargaMon[k];
